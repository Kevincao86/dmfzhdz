...
Page({
  ...
  goSchedule(e) {
    ...
    wx.navigateTo({
      url: `/pages/subpack-pr/mine-pr-order-schedule/mine-pr-order-schedule?id=${encodeURIComponent(id)}`,
    })
  },
  goScheduleReview(e) {
    const id = e.currentTarget.dataset.id
    ...
    identityTheme.applyChrome('pr', { animate: false })
    wx.navigateTo({
      url: `/pages/subpack-pr/mine-pr-order-schedule/mine-pr-order-schedule?id=${encodeURIComponent(id)}&view=review`,
    })
  },
  goViewCompleted(e) {
    const id = e.currentTarget.dataset.id
