...
Page({
  ...
  onBackList() {
    ...
    wx.navigateTo({ url: `/pages/subpack-pr/mine-pr-orders/mine-pr-orders?tab=${tab}` })
  },
  goApplicants() {
    wx.navigateTo({
      url: `/pages/subpack-pr/mine-pr-order-applicants/mine-pr-order-applicants?id=${encodeURIComponent(this.data.mpOrderId)}`,
    })
  },
  onGoModifyDates() {
    const mp = this._mp
    ...
    try {
      ...
      if (confirmEffective) {
        if (isReview) {
          ...
          wx.redirectTo({
            url:
              `/pages/subpack-pr/mine-pr-order-schedule-success/mine-pr-order-schedule-success?id=${encodeURIComponent(mpOrderId)}` +
