...
def slide_01():
    ...
    text(d, (80, 860), "种子轮  人民币 500 万元", f_body(28), WHITE)
    text(
        d,
        (80, 910),
        "温州灵祺智能科技有限公司  ·  曹鑫淼  15757468650",
        f_body(20, True),
        (200, 196, 188),
    )
    text(d, (1840, 910), "数据截至 2026-09-06 轻量只读快照", f_body(18, True), (180, 176, 168), "rt")
    return img
...
def slide_02():
    img = new_slide()
    d = ImageDraw.Draw(img)
    text(d, (80, 64), "01  /  90 秒结论", f_sans(18), RUST)
    ...
    text(
        d,
        (80, 800),
        "本轮不讲「12 个月 2 万店」。只证明：产品已上线、双边已有密度、单位经济可以在杭甬温被审计。",
        f_body(24),
        INK,
    )
    text(
...
def slide_05():
    ...
    text(d, (80, 108), "已在生产环境，不是路线图", f_title(44), INK)
    rows = [
        ...
        ("运营台 + API", "mofangdianai.com/erp-api", "内部", "212 条路由，轻量连续运行 100 天"),
    ]
    y = 210
...
def slide_06():
    ...
    text(d, (80, 108), "2026-09-06 轻量只读快照，可复核", f_title(40), INK)
    items = [
        ...
        (80, 460, 520, 680, "32", "ERP 租户", "具名主体约 14，不是 32 家付费"),
    ...
    text(d, (110, 760), "开口先报这四个数", f_sans(18), GOLD)
    text(
        d,
        (110, 810),
    ...
    text(
        d,
        (110, 880),
        "733 万是商户在抖音侧的成交，证明 ERP 吃进了真经营数据。灵祺自己的收入仍是订阅 + 履约 + 积分。",
        f_body(20, True),
        (210, 200, 188),
    )
    footer(d, 6)
...
def slide_07():
    img = new_slide()
    d = ImageDraw.Draw(img)
    text(d, (80, 64), "06  /  数字怎么拆", f_sans(18), RUST)
    text(d, (80, 108), "专业机构第一刀，我们自己先砍", f_title(40), INK)
    card(d, (80, 210, 920, 980))
    text(d, (110, 240), "ERP 租户 32 不能直接说付费", f_title(26), INK)
    rows = [
        ("具名经营 / 服务商", "14", "小七咖啡、德悦、阿毛饭店、塔塔、极道、意米、地锅鸡…"),
        ("微信快捷注册", "13", "店名未完善，获客入口在、激活不足"),
        ("测试 / 停用", "5", "对外不计入"),
    ...
    text(d, (990, 240), "3 家深度店 = 最硬需求侧证据", f_title(26), INK)
    shops = [
    ...
    for name, a, b, c in shops:
        ...
        y += 190
    text(d, (990, 900), "会员档位已开通 14 户，多为运营赠送，不能当成 ARR。", f_body(18, True), MUTED)
    footer(d, 7)
    return img
...
def slide_08():
    ...
    text(
        d,
        (80, 800),
        "高峰在夏季冷启动。8 月后转入质量运营。融资用于单城 densify，不靠全国投放。",
        f_body(22, True),
        MUTED,
    )
    text(
...
def slide_10():
    ...
    text(d, (80, 108), "定价锚点已在，规模化收费是本轮要打的洞", f_title(38), INK)
    models = [
    ...
    for a, b, c in models:
        ...
        y += 136
    text(d, (1060, 200), "500 万用途", f_title(28), INK)
    funds = [
        ("研发与稳定性", "220 万", "44%", TEAL),
        ("交付与运营", "150 万", "30%", RUST),
        ("市场与销售", "80 万", "16%", GOLD),
        ("储备与合规", "50 万", "10%", (90, 88, 82)),
    ]
    y = 270
...
def slide_11():
    ...
    text(d, (80, 108), "对外只讲 Base 轨", f_title(42), INK)
    comps = [
    ...
    for a, b, c in comps:
        ...
        x += 450
    text(d, (80, 480), "Base 里程碑（过会用）", f_title(26), INK)
    miles = [
        ("当前", "具名约 14 · 深用 3", "本快照可复核"),
        ("M3", "付费 30–80", "3 个账单级案例"),
        ("M6", "200–500", "≥3 家服务商付费导入"),
        ("M12", "1,500–3,000", "ARPU / 留存可披露"),
    ]
    x = 80
    for t, a, b in miles:
    ...
    text(
        d,
        (80, 910),
        "旧材料里的「M12 两万店 / MRR 1000 万」只作 Stretch。冷邮和路演不要写。",
        f_body(20, True),
        MUTED,
    )
    footer(d, 11)
...
def slide_12():
    ...
    text(d, (80, 108), "种子轮  人民币 500 万元", f_title(48), WHITE)
    text(
        d,
        (80, 190),
        "浙江落地资本 + 早期品牌资本拼盘即可，不必一家吃满。",
        f_body(22, True),
        (220, 214, 204),
    )
    inst = [
        ("P0  本周提交", "奇绩创坛  ·  天使湾  ·  浙商创投  ·  温州科创 / 市基金"),
    ...
    text(d, (110, 630), "温州灵祺智能科技有限公司", f_title(28), WHITE)
