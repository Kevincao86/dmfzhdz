...
def slide_01():
    ...
    text(d, (80, 430), "本地生活商家 AI ERP", f_title(46), CREAM)
    text(d, (80, 500), "加上星选达人履约", f_title(46), CREAM)
...
def slide_02():
    ...
    boxes = [
        (80, 340, 620, 720, "商家侧", "商家 / 服务商 AI ERP", "组品 · 方案 · 店铺分析\n短视频 · 投流 · 确认后写入"),
        (660, 340, 1260, 720, "打通", "同一套系统", "商家、星选、后台同源\n发单和履约对得上"),
        (1300, 340, 1840, 720, "达人侧", "星选达人履约", "招募 · 报名 · 探店 / 云剪\n回传核实 · 结算"),
    ]
    ...
    text(
        d,
        (80, 800),
        "本轮要验证三件事：产品已上线、商家和达人都有真实用量、收费能在杭甬温算清楚。",
        f_body(24),
        INK,
    )
    text(
...
def slide_03():
    ...
    pains = [
        ("不会操盘", "组品、方案、评价散落多个后台，依赖代运营口头方案"),
        ("达人难管", "微信群和私聊没有统一状态，排期对账靠人盯"),
...
def slide_04():
    ...
    text(
        d,
        (80, 180),
        "AI 不是聊天玩具。商户确认后再写入系统。履约不是只发招募，发布后能回传核实。",
        f_body(22, True),
        (220, 214, 204),
    )
    left = [
        ("商家 AI ERP", "用自然语言完成组品、方案、店铺分析"),
        ("确认后再写入", "降低误操作，商户自己把关"),
        ("多平台一起用", "来客、美团、小红书，不替代平台交易"),
    ]
    right = [
        ("星选四身份", "达人 · PR · 拍摄 · 剪辑"),
        ("探店 + 云剪", "报名、反选、交付、回传、结算"),
        ("数据打通", "商家 ERP 和星选是同一套数据"),
    ]
...
def slide_05():
    ...
    text(d, (80, 108), "四端已上线，可当面演示发单到报名", f_title(40), INK)
    rows = [
        ("商家 ERP", "cs.mofangdianai.com", "商户", "AI 组品 / 招募 / 短视频 / 店铺分析"),
        ("服务商 ERP", "fws.mofangdianai.com", "代运营", "多家客户、批量带店"),
        ("星选平台", "dr.mofangdianai.com · 微信搜灵祺星选", "达人 / PR / 团队", "大厅、急单、云剪、私信、积分"),
        ("线上系统", "已连续运行 100 天", "全链路", "登录、大厅、支付均已上线"),
    ]
    y = 210
...
def slide_08():
    ...
    text(d, (80, 108), "温州已经能看出一座城", f_title(40), INK)
    stats = [
        ("264", "温州会员"),
        ("424", "浙江会员"),
        ("170", "≥10 万粉达人"),
...
def slide_09():
    ...
    text(d, (80, 108), "先把杭甬温做深，再谈下一城", f_title(42), WHITE)
    facts = [
...
def slide_10():
    ...
    models = [
        ("商家订阅", "约 ¥168 / 月", "支付页已出现 168 档"),
        ("履约 / 撮合", "¥10–50 / 单", "大厅已有预算"),
        ("达人 / PR 会员", "约 ¥9.9 / 月", "积分月赠已跑，充值几乎为 0"),
        ("服务商席位", "1.98–4.98 万 / 年", "5 家服务商客户待转合同"),
        ("AI / 云资源", "按用量和云剪加价", "7 月 4,286 次调用可作报价"),
    ]
    y = 200
    for a, b, c in models:
...
def slide_11():
    ...
    comps = [
        ("再惠", "高客单全栈代运营", "我们：低价自助，先把一座城做深"),
        ("有赞", "经营系统 + 种草打包", "我们：自己有达人履约，能回传核实"),
        ("只发招募的工具", "只抢营销预算", "我们：经营一体，不只发单"),
        ("来客 / 美团", "平台后台够用化", "我们：多平台一起用，不碰资金"),
    ]
    x = 80
    for a, b, c in comps:
    ...
    miles = [
        ...
        ("M6", "150–300", "至少 3 家服务商付费带客"),
        ("M12", "800–1,500", "客单价 / 留存可披露"),
    ]
    x = 80
    for t, a, b in miles:
