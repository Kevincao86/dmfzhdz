...
export function enrichTalentLibraryEntry(
  ...
  const createdAt = String(entry.createdAt || member?.registeredAt || '').trim() || undefined
  if (gender && tags.length && !mpFeatureAccess && !mpMembershipPlan && !createdAt) return entry
  return {
    ...entry,
