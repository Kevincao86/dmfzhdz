...
export default async function handler(req: VercelRequest, res: VercelResponse): Promise<void> {
  ...
  if (!auth.partner.permissions.includes('dashboard')) {
    sendOpsJson(res, 403, { ok: false, code: 'permission_denied', message: '无看板权限' })
    return
  }
  ...
  try {
    ...
    sendOpsJson(res, 200, {
      ok: true,
      dashboard: buildDashboard(auth.partner, merchants, orders),
    })
  } catch (e) {
