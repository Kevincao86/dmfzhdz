...
Page({
  ...
  _syncModeHint() {
    ...
    if (mode === 'login') {
      ...
      submitLabel = loginMethod === 'password' ? '登录并进入工作台' : '验证码登录'
    }
  ...
  async onWxLogin() {
    ...
    try {
      const wxSession = await wxAccountMp.fetchWxLoginCode()
      const r = await tenantAuthApi.loginWithWxCode({
        code: wxSession.code,
        stableDevOpenId: wxSession.stableDevOpenId,
      })
