/**
 * 商家 ERP 小程序 · 菜单/竞对/数字人 TTS / AI 运营方案 API 封装
 */
...
function menuSummaryLines(items, max) {
  ...
  const lines = list.slice(0, limit).map((it) => {
    ...
    if (!name) return ''
    const p =
      it && typeof it.priceYuan === 'number' && Number.isFinite(it.priceYuan) ? ` ¥${it.priceYuan}` : ''
...
function normalizeOpsPlan(plan) {
  ...
  const talent = plan.talentBudget || {}
  return {
    ...
    talentRows: Array.isArray(talent.talentRows) ? talent.talentRows : [],
  }
}
