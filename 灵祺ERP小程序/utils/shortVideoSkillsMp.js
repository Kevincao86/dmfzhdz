/** 与 CS shortVideoSkills 同源 */
const SHORT_VIDEO_SKILLS = [
  {
    id: 'store_visit',
    name: '探店成片',
    ...
    structureBeats: ['hook','product','cta'],
    promptTemplate: [
      '【Skill·探店成片】',
  ...
  },
  {
    ...
    preferAspect: '9:16',
    tags: ['种草', '电商', '卖点'],
    briefSlots: ['scene','offer','audience'],
    structureBeats: ['hook','product','cta'],
    promptTemplate: [
      '【Skill·产品种草】',
...
const BRIEF_SLOT_LABELS = { scene: '场景/门店', offer: '卖点/主品', audience: '受众' }

function findShortVideoSkill(id) {
  if (!id) return null
  return SHORT_VIDEO_SKILLS.find((s) => s.id === id) || null
}

function matchSkillsByQuery(q) {
  const t = String(q || '').trim().toLowerCase()
  if (!t) return SHORT_VIDEO_SKILLS.slice()
...
function composeSkillPrompt(skillItem, userNote) {
...
function missingBriefSlots(skillItem, brief) {
  const slots = (skillItem && skillItem.briefSlots) || []
  const b = brief || {}
  return slots.filter((k) => !String(b[k] || '').trim()).map((k) => BRIEF_SLOT_LABELS[k] || k)
}
