...
const REGIONS = [
  ...
  {
    province: '黑龙江省',
    cities: ['哈尔滨市', '齐齐哈尔市', '鸡西市', '鹤岗市', '双鸭山市', '大庆市', '伊春市', '佳木斯市', '七台河市', '牡丹江市', '黑河市', '绥化市'],
  },
  {
    province: '江苏省',
  ...
  {
    province: '湖北省',
    cities: ['武汉市', '黄石市', '十堰市', '宜昌市', '襄阳市', '鄂州市', '荆门市', '孝感市', '荆州市', '黄冈市', '咸宁市', '随州市'],
  },
  {
    province: '湖南省',
  ...
  {
    province: '四川省',
    cities: ['成都市', '自贡市', '攀枝花市', '泸州市', '德阳市', '绵阳市', '广元市', '遂宁市', '内江市', '乐山市', '南充市', '眉山市', '宜宾市', '广安市', '达州市', '雅安市', '巴中市', '资阳市'],
  },
  {
    province: '贵州省',
  ...
  {
    province: '云南省',
    cities: ['昆明市', '曲靖市', '玉溪市', '保山市', '昭通市', '丽江市', '普洱市', '临沧市'],
  },
  {
    province: '西藏自治区',
  ...
  {
    province: '甘肃省',
    cities: ['兰州市', '嘉峪关市', '金昌市', '白银市', '天水市', '武威市', '张掖市', '平凉市', '酒泉市', '庆阳市', '定西市', '陇南市'],
  },
  {
    province: '青海省',
    cities: ['西宁市', '海东市'],
  ...
  {
    province: '新疆维吾尔自治区',
    cities: ['乌鲁木齐市', '克拉玛依市', '吐鲁番市', '哈密市', '昌吉回族自治州', '伊犁哈萨克自治州'],
  },
  { province: '香港特别行政区', cities: ['香港'] },
  { province: '澳门特别行政区', cities: ['澳门'] },
  { province: '台湾省', cities: ['台北市', '高雄市', '台中市', '台南市'] },
]
...
function cityList(province) {
  const row = REGIONS.find((r) => r.province === province)
  return row ? [...row.cities] : []
}

function validateRegion(province, city) {
  if (!String(province || '').trim()) return '请选择省份'
  if (!String(city || '').trim()) return '请选择城市'
  const cities = cityList(province)
  if (!cities.includes(city)) return '请选择有效城市'
  return null
}
...
function provinceMatchesFilter(inferredProvince, filterProvince) {
