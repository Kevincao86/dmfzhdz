...
Page({
  ...
  onAddMore() {
    ...
    wx.navigateTo({
      url: `/pages/mine-pr-targeted-pick/mine-pr-targeted-pick?id=${encodeURIComponent(mpOrderId)}&hours=${inviteResponseHours}`,
    })
  },
  onGoSchedule() {
    const { mpOrderId } = this.data
    wx.navigateTo({
      url: `/pages/mine-pr-order-schedule/mine-pr-order-schedule?id=${encodeURIComponent(mpOrderId)}`,
    })
  },
  ...
  async onConfirmInvitePhase() {
    ...
    try {
      ...
      setTimeout(() => {
        wx.redirectTo({
          url: `/pages/mine-pr-orders/mine-pr-orders?tab=pending_schedule`,
        })
      }, 500)
    } catch (e) {
