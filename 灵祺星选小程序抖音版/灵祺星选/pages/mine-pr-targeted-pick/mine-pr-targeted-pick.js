...
function matchRow(row, kw, platform, city, salesLevel) {
  ...
  if (!k) return true
  const blob = [row.name, row.id, row.region, row.city, (row.accountTags || []).join(' ')].join(' ').toLowerCase()
  return blob.includes(k)
}

function collectCityOptions(rows) {
  const set = new Set()
  for (const r of rows || []) {
...
Page({
  ...
  onLoad(query) {
    ...
    this.setData({ mpOrderId, inviteResponseHours: hours })
    this.loadPool()
  },
  async loadPool() {
    ...
    try {
      ...
      const cityOpts = collectCityOptions(rows)
      const salesLevelOpts = collectSalesLevelOptions(rows)
      this.setData({ allRows: rows, cityOpts, salesLevelOpts, loading: false })
      this.applyFilter()
    } catch (e) {
  ...
  applyFilter() {
    const { allRows, keyword, filterPlatform, filterCity, filterSalesLevel } = this.data
    const displayRows = (allRows || []).filter((r) =>
      matchRow(r, keyword, filterPlatform, filterCity, filterSalesLevel),
    )
  ...
  onFilterCityChange(e) {
    const idx = Number(e.detail.value) || 0
    const opts = this.data.cityOpts || ['全部']
    this.setData({
      filterCityIndex: idx,
      filterCity: opts[idx] || '全部',
    })
    this.applyFilter()
  },
  ...
  async onSend() {
    ...
    try {
      ...
      setTimeout(() => {
        wx.redirectTo({
          url: `/pages/mine-pr-targeted-manage/mine-pr-targeted-manage?id=${encodeURIComponent(mpOrderId)}`,
        })
      }, 400)
    } catch (e) {
