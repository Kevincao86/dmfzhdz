...
const LOCAL_SUPABASE_DEMO_SERVICE_ROLE = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZS1kZW1vIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImV4cCI6MTk4MzgxMjk5Nn0.EGIM96RAZx35lJzdJsyH-qQwv8Hdp7fsn3W0YpN81IU';
function isLocalSupabaseDemoUrl(supabaseUrl) {
    try {
...
function effectiveServiceRoleKey(supabaseUrl, fromEnv) {
    const t = fromEnv.trim();
    if (isLocalSupabaseDemoUrl(supabaseUrl)) {
        if (process.env.SUPABASE_LOCAL_USE_PRINTED_SERVICE_ROLE === '1' && t)
            return t;
        return LOCAL_SUPABASE_DEMO_SERVICE_ROLE;
    }
    return t;
}
function buildMerchantSupabaseAdminEnvParts(supabaseUrl) {
    ...
    const missingParts = [];
    if (!supabaseUrl)
        missingParts.push('url');
    if (!serviceRole)
        missingParts.push('serviceRole');
    return { supabaseUrl, serviceRole, missingParts };
}
export function readMerchantSupabaseAdminEnv() {
    const supabaseUrl = (process.env.MEOO_SUPABASE_ADMIN_URL ??
...
function isCloudSupabaseHost(url) {
    try {
 ...
 */
export function readSupportRelaySupabaseAdminEnv() {
    const explicit = process.env.MEOO_SUPABASE_ADMIN_URL?.trim().replace(/\/$/, '');
    if (explicit)
        return buildMerchantSupabaseAdminEnvParts(explicit);
    if (process.env.MEOO_AUTH_API_SERVER === '1') {
        const local = (process.env.SUPABASE_URL ??
            process.env.VITE_SUPABASE_URL ??
            'http://127.0.0.1:8888')
            .trim()
            .replace(/\/$/, '');
    ...
    const ecsPublic = (process.env.MEOO_ECS_POSTGREST_PUBLIC_URL ?? MEOO_ECS_POSTGREST_PUBLIC_DEFAULT)
        .trim()
        .replace(/\/$/, '');
    return buildMerchantSupabaseAdminEnvParts(ecsPublic);
}
/** support_relay_messages 读写：ECS 根域走备案 bypass fetch */
export function supportRelayAdminFetch(url, init) {
    return supabaseAdminFetch(url, init ?? {});
}
/** 校验用户 JWT（anon）与 Service Role 分离；Vercel 上常配 SUPABASE_ANON_KEY。 */
export function readMerchantSupabaseAnonKey() {
    return (process.env.VITE_SUPABASE_ANON_KEY ?? process.env.SUPABASE_ANON_KEY ?? '').trim();
}
/** 给人看的配置说明（JSON hint 字段等） */
export function merchantSupabaseAdminEnvConfigureHint(missingParts) {
    const lines = [
        '请在「商户 ERP」绑定的 Vercel 项目 → Settings → Environment Variables 中，为 Production（及 Preview 如需）勾选并填写：',
    ];
    if (missingParts.includes('url')) {
        lines.push('· VITE_SUPABASE_URL 或 SUPABASE_URL：与 ERP 登录页使用的 Supabase 项目 URL 一致（https://xxx.supabase.co，无末尾斜杠）。');
    }
    if (missingParts.includes('serviceRole')) {
        lines.push('· SUPABASE_SERVICE_ROLE_KEY：Supabase Dashboard → Project Settings → API →「service_role」密钥（勿用 anon key）。与运营台读同一注册表时，填同一项目下的 service_role。');
    }
    lines.push('保存后必须对商户 ERP 执行一次 Redeploy，Serverless 才会加载新变量。');
    return lines.join('');
}
export function supportRelaySupabaseEnvConfigureHint(missingParts) {
    const lines = [
        '在线客服须与小程序共用 ECS Postgres（support_relay_messages）。请在 Vercel 商家管理后台配置：',
    ];
    if (missingParts.includes('url')) {
        lines.push(`· MEOO_SUPABASE_ADMIN_URL=${MEOO_ECS_POSTGREST_PUBLIC_DEFAULT}（或轻量内网 http://127.0.0.1:8888，勿用 *.supabase.co）`);
    }
    if (missingParts.includes('serviceRole')) {
        lines.push('· SUPABASE_SERVICE_ROLE_KEY：与 ECS auth-api.env 中 service_role JWT 一致（bash scripts/ecs-run-auth-api.sh 生成）。');
    }
    lines.push('· VITE_MEEO_SUPPORT_OPS_API_BASE=https://mofangdianai.com/erp-api；MEOO_SUPPORT_OPS_HTTP_TOKEN 与 VITE_ 同名变量一致。');
