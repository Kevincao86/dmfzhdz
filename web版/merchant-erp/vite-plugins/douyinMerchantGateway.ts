...
function getDataError(j: Record<string, unknown>): { ok: boolean; msg?: string } {
  ...
  if (extra && typeof extra === 'object') {
    ...
    if (code !== undefined && code !== 0) {
      ...
      return {
        ok: false,
        msg: sub || desc || `抖音 extra error_code=${code}`,
      }
    }
  }
  return { ok: true }
...
function isDouyinOpenApiRateLimited(err: unknown): boolean {
...
function isShopQueryRateLimited(err: unknown): boolean {
  return isDouyinOpenApiRateLimited(err)
}
 ...
 * 排障：`DOUYIN_GOODS_COMBO_COMMODITY_JSON_SHAPE=flattened` 可试扁平 ItemStruct[]（多数类目不适用）。
 */
...
function amountFenFromUnknown(v: unknown): number {
  const n = Number(v)
...
/** 来客数据看板按支付时间归属；无 pay_time 时回退创单时间 */
function orderPayUnixSec(order: Record<string, unknown>): number {
  return (
    unixSecFromApiTime(order.pay_time) ||
    unixSecFromApiTime(order.create_order_time) ||
...
/** 官方订单查询只有创单/更新时间窗；向前扩几天以覆盖「头天创单、次日支付」 */
...
async function paginateDouyinTradeOrders(
  ...
  while (page <= maxPages) {
    ...
    const j = parseDouyinJson(raw)
    if (!dr.ok) {
      if (isDouyinOpenApiRateLimited(raw) && attempt < 3) {
        attempt += 1
    ...
    if (!envErr.ok) {
      if (isDouyinOpenApiRateLimited(envErr.msg ?? '') && attempt < 3) {
        attempt += 1
        await sleep(500 * attempt * attempt)
        continue
      }
...
export type DouyinFinanceReconcileOpts = {
  /** 首页看板只要成交额：跳过 update 窗，少一半周切片 */
  skipUpdateQuery?: boolean
  /** 财务管理点「刷新」时绕过短缓存 */
  bypassCache?: boolean
}

type DouyinFinanceReconcileResult = {
...
const douyinFinanceReconcileCache = new Map<string, { at: number; value: DouyinFinanceReconcileResult }>()
const douyinFinanceReconcileInflight = new Map<string, Promise<DouyinFinanceReconcileResult>>()

/**
 * 成交按支付时间 + 实收金额对齐来客后台；查询仍用创单时间窗（官方无 pay_time 筛），按周切片避免整月分页打满。
 */
export async function fetchDouyinFinanceReconcileRows(
  bearerToken: string,
  startYmd: string,
  endYmd: string,
  opts?: DouyinFinanceReconcileOpts,
): Promise<DouyinFinanceReconcileResult> {
  const warnings: string[] = []
  const session = bearerToken ? resolveSession(bearerToken) : undefined
  if (!session) {
  ...
  const cacheKey = `${session.merchantId}|${startYmd}|${endYmd}|${opts?.skipUpdateQuery ? 'sales' : 'full'}`
  if (!opts?.bypassCache) {
    const hit = douyinFinanceReconcileCache.get(cacheKey)
    if (hit && Date.now() - hit.at < DOUYIN_FINANCE_CACHE_TTL_MS) return hit.value
    const inflight = douyinFinanceReconcileInflight.get(cacheKey)
    if (inflight) return inflight
  }
  const work = loadDouyinFinanceReconcileRows(session, startYmd, endYmd, warnings, opts)
  douyinFinanceReconcileInflight.set(cacheKey, work)
  try {
    const value = await work
    douyinFinanceReconcileCache.set(cacheKey, { at: Date.now(), value })
    return value
  } finally {
    douyinFinanceReconcileInflight.delete(cacheKey)
  }
}

async function loadDouyinFinanceReconcileRows(
  ...
  try {
    ...
    for (const chunk of eachShanghaiWeekChunks(queryStart, endYmd)) {
      ...
      if (chunkWarnings.some((w) => w.includes('分页达到上限'))) {
        hitPageCap = true
        warnings.push(`区间 ${chunk.start}~${chunk.end} 订单较多，该周可能未拉全。`)
      } else {
        for (const w of tradeWarnings) {
          if (!warnings.includes(w)) warnings.push(w)
        }
        if (!skipHermes) {
          for (const w of hermesWarnings) {
            if (!warnings.includes(w)) warnings.push(w)
          }
        }
      }
    ...
    if (!opts?.skipUpdateQuery) {
      for (const chunk of eachShanghaiWeekChunks(startYmd, endYmd)) {
        ...
        if (chunkWarnings.some((w) => w.includes('分页达到上限'))) {
          hitPageCap = true
          warnings.push(`核销更新区间 ${chunk.start}~${chunk.end} 订单较多，该周可能未拉全。`)
        } else {
          for (const w of chunkWarnings) {
            if (!warnings.includes(w)) warnings.push(w)
          }
    ...
    if (hitPageCap) {
      warnings.push('部分周次触及分页上限；已按周切片尽量拉全。可改用更短日期再查。')
    }
  } catch (e) {
    const msg = e instanceof Error ? e.message : String(e)
  ...
  if (!warnings.some((w) => w.startsWith('抖音：成交'))) {
    warnings.push(
      '抖音：成交按支付时间、实收金额（receipt_amount）对齐来客后台；核销仅统计券 item_status=401（已履约），按 item_update_time 归属日期。',
    )
  }
  return {
    rows,
 ...
 * 勿用 /100 启发式（会把 100 误判为 1 星，导致好评显示差评）。
 */
function akteRateScoreToStars(rateScore: unknown): number {
  const raw = stringifyDouyinOpenApiInt64(rateScore)
  if (!raw) return 0
  const n = Number(raw)
  if (!Number.isFinite(n) || n <= 0) return 0
...
export { douyinPoiIdsMatch } from '../api/douyinOpenApiBase.js'
  ...
  for (const win of timeWindows) {
    ...
    for (let page = 0; page < maxPages; page += 1) {
      ...
      for (let attempt = 0; attempt < 6; attempt += 1) {
        if (attempt > 0) {
          const waitMs = AKTE_COMMENT_RATE_LIMIT_BACKOFF_MS * 2 ** (attempt - 1)
        ...
        if (!dr.ok) {
          ...
          if (attempt < 5 && isDouyinOpenApiRateLimited(lastErr)) continue
...
async function fetchAkteCommentsForTargetWithRetry(
  accessToken: string,
  accountId: string,
  target: { poiId?: string; productId?: string; poiIds?: string[]; productIds?: string[] },
  ctx: { reviewKind: 'store' | 'product'; poiId?: string; poiName?: string; productId?: string; productName?: string },
  ...
  return last ?? { ok: false, message: '评价查询失败' }
}
...
export async function fetchDouyinAkteReviews(
  ...
  | { ok: true; items: MerchantReviewRowDouyin[]; warning?: string }
      ...
      for (let i = 0; i < productTargets.length; i += 20) {
        ...
        if (r.ok === false) {
          if (merged.length > 0) {
            return {
              ok: true,
              items: sortDouyinReviewsByNewest(merged),
              warning: `已同步 ${merged.length} 条；部分商品批次未完成（${r.message}）。请稍后重试。`,
            }
          }
          return r
        }
