...
export function resolveIceEffectPreset(id: string): IceEffectPreset {
  const t = id.trim()
  const hit = ICE_EFFECT_PRESETS.find((p) => p.id === t)
  if (hit) return hit
  const byLabel = ICE_EFFECT_PRESETS.find((p) => p.label === t)
