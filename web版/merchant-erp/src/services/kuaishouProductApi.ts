 ...
 * @see https://developer.open-kuaishou.com/docs/resource/zh-CN/local-life/develop/OpenAPI/general-capabilities/product-query/category.get
 * @see https://developer.open-kuaishou.com/docs/resource/zh-CN/local-life/develop/OpenAPI/general-capabilities/product-query/template.get
 * @see https://developer.open-kuaishou.com/docs/resource/zh-CN/local-life/develop/OpenAPI/general-capabilities/goods/save
 */
...
export type KuaishouCategoryTreeNode = {
  category_id: string
  name: string
  parent_id: string
  level: number
  ...
  sub_tree_infos?: KuaishouCategoryTreeNode[]
}
...
function normalizeCategoryNode(raw: Record<string, unknown>): KuaishouCategoryTreeNode {
...
export function normalizeCategoryTree(raw: unknown[]): KuaishouCategoryTreeNode[] {
...
export function collectUploadableLeafCategoryIdsFromTree(
  nodes: KuaishouCategoryTreeNode[],
): string[] {
  const out: string[] = []
  const walk = (arr: KuaishouCategoryTreeNode[]) => {
    for (const n of arr) {
...
export async function uploadKuaishouProductImage(file: File): Promise<ImageUploadResult> {
  ...
  const paths = ['/api/meoo-kuaishou-goods-image-upload', '/api/merchant/kuaishou/goods/image/upload'] as const
  ...
  return {
    ...
    message:
      lastStatus === 404
        ? '图片上传接口返回 404：请部署含 /api/meoo-kuaishou-goods-image-upload 的版本，或检查 VITE_MERCHANT_API_BASE_URL。'
        : `HTTP ${lastStatus || 404}`,
  }
}
...
export type CategoryGetResult =
  | { ok: false; message: string }
  | { ok: true; category_tree_infos: KuaishouCategoryTreeNode[] }
...
export type KuaishouProductDetailPayload = {
  /** 已存在商品时传入，网关走更新/覆盖草稿 */
  product_id?: string
  out_id: string
  category_id: string
  ...
  /**
   * 按 [template.get](https://developer.open-kuaishou.com/docs/resource/zh-CN/local-life/develop/OpenAPI/general-capabilities/product-query/template.get)
   * 返回的 attr `key` 直填 `attr_key_value_map`（opaque key、combo_rule 类字段等），由网关合并进 [product.save](https://developer.open-kuaishou.com/docs/resource/zh-CN/local-life/develop/OpenAPI/general-capabilities/goods/save)。
   */
...
export type KuaishouGoodsProductGetResult =
  | { ok: true; detail: KuaishouProductDetailPayload }
  | { ok: false; message: string }

/**
 * 商品查询：对齐网关代理的 goodlife 商品详情/草稿查询（路径约定，生产由网关实现）。
 */
export async function getKuaishouGoodsProductGet(productId: string): Promise<KuaishouGoodsProductGetResult> {
  const id = productId.trim()
  if (!id) return { ok: false, message: '缺少 product_id' }
  const q = new URLSearchParams({ product_id: id })
  appendKuaishouAccountIdToQuery(q)
  const qs = `?${q}`
  const paths = [
    `/api/meoo-kuaishou-goods-product-get${qs}`,
    `/api/merchant/kuaishou/goods/product/get${qs}`,
  ] as const
  const headers = authHeaders()
  let lastStatus = 0
  for (const p of paths) {
    ...
    const detail = d.detail as KuaishouProductDetailPayload | undefined
  ...
  return {
    ...
    message:
      lastStatus === 404
        ? '商品详情接口返回 404：请部署含 /api/meoo-kuaishou-goods-product-get 的版本，或检查 VITE_MERCHANT_API_BASE_URL。'
        : `HTTP ${lastStatus || 404}`,
  }
}
...
export async function postKuaishouGoodsProductSync(productId: string): Promise<ProductSyncResult> {
  const id = productId.trim()
  if (!id) return { ok: false, message: '缺少 product_id' }
  const bodyStr = JSON.stringify({ product_id: id })
  const headers = authHeaders()
  const paths = ['/api/meoo-kuaishou-goods-product-sync', '/api/merchant/kuaishou/goods/product/sync'] as const
  const targets = merchantApiFetchUrlCandidates(paths)
  let lastStatus = 0
  for (const target of targets) {
    const res = await fetch(target, { method: 'POST', headers, body: bodyStr })
  ...
  return {
    ...
    message:
      lastStatus === 404
        ? '同步接口返回 404：请部署含 /api/meoo-kuaishou-goods-product-sync 的版本，或检查 VITE_MERCHANT_API_BASE_URL。'
        : `HTTP ${lastStatus || 404}`,
  }
}
...
export type KuaishouOnlineProductHit = {
  product_id: string
  product_name: string
  sku_id?: string
...
function normalizeOnlineProductEntry(raw: unknown): KuaishouOnlineProductHit | null {
...
function kuaishouDataBizError(inner: Record<string, unknown> | undefined): string | undefined {
...
function extractProductsArrayFromKuaishouPayload(data: Record<string, unknown>): unknown[] {
...
function appendKuaishouAccountIdToQuery(qs: URLSearchParams): void {
  const id = readMerchantSession('meoo_kuaishou_merchant_id')
  if (id) qs.set('account_id', id)
}

function mergeDedupeProductHits(batches: KuaishouOnlineProductHit[][]): KuaishouOnlineProductHit[] {
  const seen = new Set<string>()
  const out: KuaishouOnlineProductHit[] = []
...
function parseOnlineQueryResponse(
...
): { hits: KuaishouOnlineProductHit[]; httpErr?: string; bizErr?: string } {
  if (!res.ok) {
    return {
      hits: [],
  ...
  const inner = data.data as Record<string, unknown> | undefined
  const bizErr = kuaishouDataBizError(inner)
  const products = extractProductsArrayFromKuaishouPayload(data)
  const hits: KuaishouOnlineProductHit[] = []
...
async function fetchOnlineQueryHits(
  qs: URLSearchParams,
): Promise<{ hits: KuaishouOnlineProductHit[]; httpErr?: string; bizErr?: string }> {
  const q = new URLSearchParams(qs.toString())
  appendKuaishouAccountIdToQuery(q)
  const qsStr = q.toString()
  const paths = [
    '/api/meoo-kuaishou-goods-product-online-query',
    '/api/merchant/kuaishou/goods/product/online/query',
  ] as const
  const headers = authHeaders()
  for (const p of paths) {
...
async function fetchDraftQueryHitsFiltered(keyword: string, count: number): Promise<KuaishouOnlineProductHit[]> {
  const q = new URLSearchParams()
  q.set('count', String(Math.min(50, Math.max(5, count))))
  appendKuaishouAccountIdToQuery(q)
  const qsStr = q.toString()
  const paths = [
    '/api/meoo-kuaishou-goods-product-draft-query',
    '/api/merchant/kuaishou/goods/product/draft/query',
  ] as const
  const headers = authHeaders()
  let data: Record<string, unknown> = {}
...
async function fetchLocalSavedGoodsHits(keyword: string): Promise<KuaishouOnlineProductHit[]> {
  const q = new URLSearchParams({
    page: '1',
    page_size: '50',
  ...
  appendKuaishouAccountIdToQuery(q)
  const qs = `?${q}`
  const paths = [`/api/meoo-kuaishou-goods-products${qs}`, `/api/merchant/kuaishou/goods/products${qs}`] as const
  const targets = merchantApiFetchUrlCandidates(paths)
 ...
 * @see https://developer.open-kuaishou.com/docs/resource/zh-CN/local-life/develop/OpenAPI/general-capabilities/product-query/online.query
 * @see https://developer.open-kuaishou.com/docs/resource/zh-CN/local-life/develop/OpenAPI/general-capabilities/product-query/query
 */
export async function getKuaishouGoodsProductOnlineQuery(params: {
  product_name: string
  count?: number
  cursor?: string
  goods_query_type?: string
}): Promise<
  | { ok: true; hits: KuaishouOnlineProductHit[]; next_cursor?: string }
  | { ok: false; message: string }
> {
  ...
  let onlineHits: KuaishouOnlineProductHit[] = []
...
/** 优先顶层 meoo 路由（与 meoo-kuaishou-stores 同理），404/HTML 时再试 merchant 深层路径 */
async function fetchCategoryGetPreferFlatPath(q: URLSearchParams): Promise<{
  res: Response
  parsed: CategoryGetParsed
}> {
  const qs = `?${q}`
  const paths = ['/api/meoo-kuaishou-goods-category-get', '/api/merchant/kuaishou/goods/category/get'] as const
  for (const p of paths) {
...
export async function getKuaishouIndustryCategoryScope(): Promise<IndustryScopeResult> {
  const accountId = readMerchantSession('meoo_kuaishou_merchant_id')
  const q = accountId ? `?account_id=${encodeURIComponent(accountId)}` : ''
  const res = await fetch(url(`/api/merchant/kuaishou/goods/industry-scope${q}`), {
    method: 'GET',
    headers: authHeaders(),
  })
  const data = await parseJson(res)
...
/** 对齐 category/get 树形结果（单请求；需补全子树请用 getKuaishouGoodsCategoryTreeMerged） */
export async function getKuaishouGoodsCategoryTree(): Promise<CategoryGetResult> {
  const accountId = readMerchantSession('meoo_kuaishou_merchant_id')
  const q = new URLSearchParams()
...
function categoryNodeNeedsSubtreeFetch(n: KuaishouCategoryTreeNode): boolean {
...
async function fetchCategorySubtreeByParentId(parentCategoryId: string): Promise<KuaishouCategoryTreeNode[]> {
  const accountId = readMerchantSession('meoo_kuaishou_merchant_id')
  const parentStr = parentCategoryId.trim()
  if (!parentStr) return []

  const runQuery = async (queryCategoryType: '0' | '1'): Promise<KuaishouCategoryTreeNode[]> => {
    const q = new URLSearchParams()
...
function cloneCategoryTree(nodes: KuaishouCategoryTreeNode[]): KuaishouCategoryTreeNode[] {
  return nodes.map((n) => ({
    ...n,
...
export function mergeKuaishouCategoryChildrenIntoTree(
  tree: KuaishouCategoryTreeNode[],
  parentCategoryId: string,
  children: KuaishouCategoryTreeNode[],
): KuaishouCategoryTreeNode[] {
...
export async function fetchKuaishouGoodsCategoryChildren(
  parentCategoryId: string,
): Promise<KuaishouCategoryTreeNode[]> {
...
async function enrichCategoryTreeMissingSubtrees(
  roots: KuaishouCategoryTreeNode[],
  options: { maxExtraRequests: number; delayMs: number },
): Promise<void> {
  let used = 0
  const queued = new Set<string>()
  const walk = async (nodes: KuaishouCategoryTreeNode[]) => {
 ...
 */
export async function getKuaishouGoodsCategoryTreeMerged(
  ...
  if (!base.ok) {
    base = await getKuaishouGoodsCategoryTree()
  }
...
export async function getKuaishouProductTypesForCategory(
  leafCategoryId: string,
): Promise<ProductTypesResult> {
  const qs = new URLSearchParams({ category_id: leafCategoryId }).toString()
  const paths = ['/api/meoo-kuaishou-goods-product-types', '/api/merchant/kuaishou/goods/product-types'] as const
  const headers = authHeaders()
  for (const p of paths) {
...
export async function getKuaishouGoodsTemplate(params: {
  category_id: string
  product_type: number
  /**
  ...
  appendKuaishouAccountIdToQuery(q)
  const qs = `?${q}`
  const paths = ['/api/meoo-kuaishou-goods-template-get', '/api/merchant/kuaishou/goods/template/get'] as const
  const headers = authHeaders()
  let lastHttpMessage = ''
  ...
  for (const p of paths) {
    ...
    for (const target of targets) {
      try {
        ...
        return mapKuaishouGoodsTemplatePayload(
          data,
          params.product_type,
          params.category_id,
...
function mapKuaishouGoodsTemplatePayload(
  data: Record<string, unknown>,
  productType: number,
  categoryId: string,
  allowEmptyTemplate?: boolean,
  ...
  if (innerEc !== undefined && innerEc !== 0) {
    ...
    const comboQtyHint =
      /数量必须大于0|单位必须为份/i.test(base) || /数量必须大于0|单位必须为份/i.test(sub)
        ? ' 说明：请核对 sku.commodity 与 attr.combo_rule 每组 item_list 是否含 count>0、unit=份、count_unit=份（见开放平台 goods/save 文档）。若手填了「字面量 JSON 覆盖」，保存时会优先采用 commodity/combo_rule 覆盖并同步顶层 product.combo_rule。模板若含 limit_rule/settle_type/use_type 须一并填写。文档：developer.open-kuaishou.com → 生活服务 → goods/save、template.get。'
        : ''
...
export async function postKuaishouGoodsProductSave(params: {
  mode: 'draft' | 'submit'
  detail: KuaishouProductDetailPayload
  ...
  const paths = ['/api/meoo-kuaishou-goods-product-save', '/api/merchant/kuaishou/goods/product/save'] as const
  const targets = merchantApiFetchUrlCandidates(paths)
  let lastStatus = 0
  let lastTarget = ''
  for (const target of targets) {
    ...
    try {
      ...
      return {
        ok: false,
        message: `保存请求未到达服务器（${msg}）。请检查网络；并在开发者工具 Network 中确认 POST 的 URL 是否为本站的 /api/meoo-kuaishou-goods-product-save（勿把 VITE_MERCHANT_API_BASE_URL 指到旧域名）。客户端 trace：${clientTrace}`,
      }
    }
    lastStatus = res.status
    const text = await res.text()
  ...
  return {
    ...
    message:
      (lastStatus === 404
        ? '保存接口返回 404：请确认商户 ERP 已部署含「/api/meoo-kuaishou-goods-product-save」的版本并已 Redeploy；若配置了 VITE_MERCHANT_API_BASE_URL，请改为当前站点或留空，以免请求打到不含该接口的旧网关。'
