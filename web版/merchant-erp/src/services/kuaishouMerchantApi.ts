/**
 * 快手团购商家版 — 与后端约定的绑定、门店同步接口。
 *
 * 门店列表须由后端代理快手官方「查询门店信息」接口（与来客认领门店一致），勿在前端直连快手。
 * 官方文档：https://developer.open-kuaishou.com/docs/resource/zh-CN/local-life/develop/OpenAPI/general-capabilities/life.capacity.shop/store-management/shop.query
 * 接口：GET https://open.kwailocallife.com/goodlife/v1/shop/poi/query/
 ...
 *
 * 门店品牌列表：GET /api/merchant/kuaishou/brands → 代理 goodlife/v2/shop/brand/query/（与来客「门店品牌」一致，非门店名称）
 *
 * 部署：绑定依次尝试 `POST /api/kuaishou-bind`、`/api/meoo-kuaishou-bind`、`/api/merchant/kuaishou/bind`（跳过 HTML/404 等基础设施响应）。
 * 门店/品牌等 GET：优先顶层 `/api/meoo-kuaishou-*`（与 ping 同级），再回退 `/api/merchant/kuaishou/*`——生产上多层 merchant 路径曾被 SPA 改写为 HTML。
 * 生产须配置 MERCHANT_KUAISHOU_SESSION_SECRET。
...
/** 浏览器 → 同源绑定接口；服务端再调快手（服务端约 45s 超时），此处略放宽避免永久挂起 */
const KUAISHOU_BIND_CLIENT_TIMEOUT_MS = 70_000
const KUAISHOU_STORES_CLIENT_TIMEOUT_MS = 65_000

function storesFetchTimeoutSignal(timeoutMs = KUAISHOU_STORES_CLIENT_TIMEOUT_MS): { signal: AbortSignal; clear: () => void } {
  const ctrl = new AbortController()
  const id = window.setTimeout(() => ctrl.abort(), timeoutMs)
  return { signal: ctrl.signal, clear: () => window.clearTimeout(id) }
}

export const KUAISHOU_SHOP_POI_QUERY_DOC =
  'https://developer.open-kuaishou.com/docs/resource/zh-CN/local-life/develop/OpenAPI/general-capabilities/life.capacity.shop/store-management/shop.query'

export type KuaishouBindPayload = {
  appId: string
  appSecret: string
  merchantId: string
}

export type KuaishouBindResult =
  | { ok: true; accessToken: string; accountName?: string; message?: string }
  | { ok: false; message: string }

export type KuaishouStoreRow = {
  id: string
  name: string
  address?: string
  city?: string
...
export type KuaishouStoresResult =
  | {
      ok: true
      items: KuaishouStoreRow[]
      total: number
      accountName?: string
      /** 已认领 / 认领中 数量，由网关聚合快手认领相关接口后返回 */
      tabCounts?: StoreTabCounts
      /** 网关返回：快手侧 relation_type 部分失败时的可读摘要 */
...
export async function postKuaishouBind(
  payload: KuaishouBindPayload,
): Promise<KuaishouBindResult> {
  ...
  const ctrl = new AbortController()
  const timeoutId = setTimeout(() => ctrl.abort(), KUAISHOU_BIND_CLIENT_TIMEOUT_MS)
  const clearBindTimer = () => clearTimeout(timeoutId)

  /** 扁平 `/api/kuaishou-bind` 优先：最少间接、最易被边缘路由命中 */
  const bindPaths = ['/api/kuaishou-bind', '/api/meoo-kuaishou-bind', '/api/merchant/kuaishou/bind'] as const
  ...
  try {
    ...
    if (aborted) {
      return {
        ok: false,
        message: `绑定请求超时（${Math.round(KUAISHOU_BIND_CLIENT_TIMEOUT_MS / 1000)} 秒）。请检查网络或稍后重试；部署在海外时请将 Functions 区域调至离大陆更近的节点（如东京 hnd1）。`,
      }
    }
    const msg =
...
export async function getKuaishouStores(params: {
  accessToken: string
  page: number
  pageSize: number
  keyword?: string
  /** 本地生活商家账户 ID（来客「账户 ID」），对应快手 account_id */
  merchantId?: string
  /** 已认领 / 认领中，须与网关分页一致（勿仅在当前页前端过滤） */
  claimScope?: 'claimed' | 'claiming'
  /** 账户与门店关系：认领 0 / 关联 1 / 挂靠 2 / 全部（网关合并去重） */
  relationType?: '0' | '1' | '2' | 'all'
  /** 为 true 时通知网关丢弃 POI 缓存并重新拉取快手 */
  refresh?: boolean
  /** 省 / 市 / 区关键词，匹配地址聚合字段 */
  provinceCity?: string
  ...
  /* 后端应将 merchantId 映射为快手 account_id，并代理 GET goodlife/v1/shop/poi/query/。
   * 官方单次 size 最大 50：若账户门店数＞50，须循环翻页直至取完再合并为 ERP 的 items/total，并返回 accountName（或由各条 poi.account.root_account.account_name 解析）。 */

  const qs = `?${q}`
  const storePaths = ['/api/meoo-kuaishou-stores', '/api/merchant/kuaishou/stores'] as const
  let res: Response | null = null
  let rawText = ''
  const { signal, clear: clearStoresTimer } = storesFetchTimeoutSignal(
    params.clientTimeoutMs ?? KUAISHOU_STORES_CLIENT_TIMEOUT_MS,
  )
  try {
    for (const p of storePaths) {
      ...
      try {
        ...
        if (name === 'AbortError') {
          return {
            ok: false,
            message: `门店列表请求超时（${Math.round(KUAISHOU_STORES_CLIENT_TIMEOUT_MS / 1000)} 秒）。绑定凭据仍保留，请稍后重试或检查网络。`,
          }
        }
        throw e
      }
  ...
  if (!res) {
    return {
      ok: false,
      message:
        '门店接口返回了 HTML 而非 JSON（已尝试 meoo 顶路径与 merchant 路径）。请部署含 api/meoo-kuaishou-stores 与 vercel.json 中 /api/(.*) 放行规则。',
    }
  }
...
export type KuaishouMerchantBrandRow = {
  brandId: string
  brandName: string
}
...
function normalizeMerchantBrandRow(row: unknown): KuaishouMerchantBrandRow | null {
...
function kuaishouBrandDataErrorMessage(d: Record<string, unknown>): string | undefined {
  const c = d.error_code
  if (typeof c === 'number' && c !== 0) {
    return typeof d.description === 'string' ? d.description : `快手业务错误 ${c}`
  }
  return undefined
}

function parseKuaishouMerchantBrandResponse(raw: Record<string, unknown>): {
  rows: unknown[]
  total: number
  errorMessage?: string
} {
  ...
  let err = kuaishouBrandDataErrorMessage(layer)
  ...
  if (
    ...
    err = kuaishouBrandDataErrorMessage(inner)
    if (err) return { rows: [], total: 0, errorMessage: err }
...
/** 单页：GET /api/merchant/kuaishou/brands → goodlife/v2/shop/brand/query/ */
export async function getKuaishouMerchantBrands(params: {
  accessToken: string
  merchantId?: string
  page?: number
  pageSize?: number
  ...
  { ok: true; items: KuaishouMerchantBrandRow[]; total: number } | { ok: false; message: string }
> {
  const q = new URLSearchParams()
  ...
  const qs = `?${q}`
  const brandPaths = ['/api/meoo-kuaishou-brands', '/api/merchant/kuaishou/brands'] as const
  let res: Response | null = null
  let rawText = ''
  ...
  if (!res) {
    return {
      ok: false,
      message: '品牌接口不可用（均为 HTML 或非 JSON）。请确认已部署 /api/meoo-kuaishou-brands。',
    }
  }
  if (!res.ok) {
    const msg =
  ...
  const parsed = parseKuaishouMerchantBrandResponse(raw)
  if (parsed.errorMessage) return { ok: false, message: parsed.errorMessage }
  const items: KuaishouMerchantBrandRow[] = []
...
export async function fetchKuaishouMerchantBrandOptions(params: {
  accessToken: string
  merchantId?: string
  maxPages?: number
}): Promise<{ ok: true; items: KuaishouMerchantBrandRow[] } | { ok: false; message: string }> {
  const pageSize = 50
  const maxPages = params.maxPages ?? 20
  const seen = new Set<string>()
  const out: KuaishouMerchantBrandRow[] = []

  for (let page = 1; page <= maxPages; page++) {
    const r = await getKuaishouMerchantBrands({
...
export type KuaishouStoreDetailResult =
  | (Extract<KuaishouStoresResult, { ok: true }> & {
      /** 快手 goodlife/v1/poi/cert/info 完整包络（与 data 并列） */
      certInfo?: Record<string, unknown>
      certInfoError?: string
      /** 快手 goodlife/v1/poi/task/query 完整包络 */
      taskQuery?: Record<string, unknown>
      taskQueryError?: string
    })
  | Extract<KuaishouStoresResult, { ok: false }>

/** 单店：GET /api/merchant/kuaishou/stores/detail?poiId= &taskIds=（可选，逗号分隔异步任务 ID） */
export async function getKuaishouStoreDetail(params: {
  accessToken: string
  poiId: string
  /** 查询 poi.update / poi.claim 等异步任务结果，见 task.query 文档 */
  taskIds?: string
}): Promise<KuaishouStoreDetailResult> {
  const q = new URLSearchParams({ poiId: params.poiId.trim() })
  const tid = params.taskIds?.trim()
  if (tid) q.set('taskIds', tid)
  const qs = `?${q}`
  const detailPaths = ['/api/meoo-kuaishou-store-detail', '/api/merchant/kuaishou/stores/detail'] as const
  let res: Response | null = null
  let rawText = ''
  ...
  if (!res) {
    return { ok: false, message: '门店详情接口不可用（均为 HTML）。请确认已部署 /api/meoo-kuaishou-store-detail。' }
  }
  if (!res.ok) {
    const msg =
      (typeof data.message === 'string' && data.message) ||
...
export type KuaishouPoiClaimResult =
  | { ok: true; status: number; bodyText: string }
  | { ok: false; message: string }

/**
 * 代理 POST goodlife/v1/poi/poi/claim/，请求体须与官方一致（含 datas 等）。
 * @see https://developer.open-kuaishou.com/docs/resource/zh-CN/local-life/develop/OpenAPI/general-capabilities/life.capacity.shop/poi.claim
 */
export async function postKuaishouPoiClaim(params: {
  accessToken: string
  body: Record<string, unknown>
}): Promise<KuaishouPoiClaimResult> {
  try {
    const claimPaths = ['/api/meoo-kuaishou-poi-claim', '/api/merchant/kuaishou/stores/poi/claim'] as const
    ...
    if (!res) {
      return { ok: false, message: '认领接口不可用（返回 HTML）。请确认已部署 /api/meoo-kuaishou-poi-claim。' }
    }
    if (!res.ok) {
 ...
 * 美团/小红书门店管理、认领列表接口经后端代理后，建议返回与本项目快手门店列表相同字段约定。
 */
export function adaptMerchantStoresPayload(data: Record<string, unknown>): {
  ...
  const items: KuaishouStoreRow[] = rawRows.map((row) => normalizeStoreRow(row))
  const total =
...
/**
 * 分页拉取快手团购「已认领」门店（relation 全量由网关合并），用于 GEO 按账户或品牌聚合。
 */
export async function fetchAllKuaishouClaimedStoresPages(params: {
  accessToken: string
  merchantId?: string
  storeBrand?: string
  keyword?: string
  ...
  | { ok: true; items: KuaishouStoreRow[]; total: number; accountName?: string; tabCounts?: StoreTabCounts }
  | { ok: false; message: string }
> {
  const pageSize = Math.min(50, Math.max(1, params.pageSize ?? 50))
  const maxPages = Math.min(60, Math.max(1, params.maxPages ?? 40))
  const items: KuaishouStoreRow[] = []
  let total = 0
...
/** 与快手 data.pois[].account.root_account.account_name 或网关顶层 accountName 对齐 */
...
/** 从常见嵌套结构中再取一层营业电话（快手各版本 POI 字段差异大） */
...
/** 快手 goodlife/v1/shop/poi/query：门店挂靠账户名，多为来客侧对客品牌（如「魔楽斑马」），非 root_account 执照主体 */
...
function normalizeStoreRow(row: unknown): KuaishouStoreRow {
