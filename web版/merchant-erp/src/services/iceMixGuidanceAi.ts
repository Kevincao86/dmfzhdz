...
5. 叙事节奏与情绪（成片须有逻辑，时长由用户参数决定，勿按素材条数堆砌镜头）：
   - 若素材含门头/店招/门店外观：优先采用「门头门店指引(开场) → 套餐/产品/制作(中段) → 结束语/行动号召(收尾)」
   - 或采用「产品/套餐卖点钩子(开场) → 体验/制作过程(中段) → 门头到店指引(收尾前) → 结束语」
   - 口播句须可对应具体画面类型（门头、菜品特写、后厨、试吃等）
   - 与推广/产品/门店毫无关联的素材（纯风景、马路、截帧失败等）不得写入指导文案，后续成片也会自动跳过
