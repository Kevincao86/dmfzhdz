/**
 * 快手团购创建/编辑商品（按官方 template.get + product/save 流程重建）。
...
import {
  loadDouyinWizardLastContext as loadKuaishouWizardLastContext,
  saveDouyinWizardLastContext as saveKuaishouWizardLastContext,
} from '../../lib/douyinWizardLastContext'
import { readMerchantSession } from '../../lib/merchantSession'
import {
  loadDouyinGoodsCategoryTreeForPicker as loadKuaishouGoodsCategoryTreeForPicker,
  pickerChildrenOf,
...
import { normalizeDouyinDescription as normalizeKuaishouDescription } from '../../lib/douyinDescriptionNormalize'
import KuaishouProductWizardDetail from './KuaishouProductWizardDetail'
import {
  fetchKuaishouGoodsCategoryChildren,
  getKuaishouGoodsProductGet,
  getKuaishouGoodsTemplate,
  getKuaishouProductTypesForCategory,
  mergeKuaishouCategoryChildrenIntoTree,
  postKuaishouGoodsProductSave,
  uploadKuaishouProductImage,
  type KuaishouCategoryTreeNode,
...
export type KuaishouProductWizardProps = {
  variant?: 'create' | 'edit'
...
function readAccountName(): string {
  return readMerchantSession('meoo_kuaishou_account_name') ?? ''
}
...
export default function KuaishouProductCreateWizard({
  variant = 'create',
  editProductId,
  autoSubmit = false,
}: KuaishouProductWizardProps = {}) {
  ...
  const [tree, setTree] = useState<KuaishouCategoryTreeNode[]>([])
  /** 随 mergeKuaishouCategoryChildrenIntoTree 更新，避免懒加载三级后 uploadableLeaves 仍为首包快照 */
  ...
  const [nonConsumeWeekdays, setNonConsumeWeekdays] = useState<KuaishouWeekdayKey[]>([])
  ...
  const [dailyTimePeriods, setDailyTimePeriods] = useState<KuaishouTimePeriod[]>([
  ...
  const buildFormRules = useCallback((): KuaishouProductFormRules => {
    return {
      salesChannel,
      saleTimeLimited,
  ...
  useEffect(() => {
    ...
    void (async () => {
      setLoading(true)
      const r = await loadKuaishouGoodsCategoryTreeForPicker()
      if (cancelled) return
      setLoading(false)
      if (!r.ok) {
  ...
  useEffect(() => {
    ...
    if (!draft) return
    const lastCtx = loadKuaishouWizardLastContext()
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      if (!detail) {
        const r = await getKuaishouGoodsProductGet(key)
        if (cancelled) return
        if (!r.ok) {
          setLoading(false)
          setLoadErr(r.message)
  ...
  useEffect(() => {
    ...
    let cancelled = false
    void fetchKuaishouGoodsCategoryChildren(cat1).then((kids) => {
      if (cancelled || kids.length === 0) return
      setTree((prev) => mergeKuaishouCategoryChildrenIntoTree(prev, cat1, kids))
    })
    return () => {
      cancelled = true
    }
  ...
  useEffect(() => {
    ...
    void fetchKuaishouGoodsCategoryChildren(cat2).then((kids) => {
      if (cancelled) return
      setCatSubtreeLoading(null)
      if (kids.length > 0) {
        setTree((prev) => mergeKuaishouCategoryChildrenIntoTree(prev, cat2, kids))
      }
    })
    return () => {
      cancelled = true
  ...
  useEffect(() => {
    ...
    void getKuaishouProductTypesForCategory(cat3).then((r) => {
      if (cancelled) return
  ...
  const onPickImage = useCallback(
    async (file: File, slot: 'head' | 'aux' | 'env', index = 0) => {
      setUploading(true)
      const r = await uploadKuaishouProductImage(file)
      setUploading(false)
      if (!r.ok) {
  ...
  const buildPayload = useCallback((): KuaishouProductDetailPayload | null => {
    const cat = cat3.trim()
  ...
  const handleSave = async (mode: 'draft' | 'submit') => {
    ...
    if (mode === 'submit') {
      const descCheck = normalizeKuaishouDescription(
        detail.product_desc ?? '',
        detail.product_name,
        undefined,
        undefined,
    ...
    setSaving(true)
    let r: Awaited<ReturnType<typeof postKuaishouGoodsProductSave>>
    try {
      r = await postKuaishouGoodsProductSave({ mode, detail })
    } catch (e) {
      r = {
        ok: false,
  ...
  const goDetail = async () => {
    ...
    try {
      const tpl = await getKuaishouGoodsTemplate({
        category_id: cat3,
        product_type: productType,
      ...
      {step === 'category' && (
        <section className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm space-y-4">
          ...
          <p className="text-sm text-gray-500">
            须选至末级类目（部分行业仅两级，选完二级后会自动带出末级）。数据来自快手{' '}
