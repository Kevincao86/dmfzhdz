...
import { applyActiveKuaishouBinding } from '../../lib/kuaishouActiveBinding'
import {
...
import {
  deleteKuaishouBindingCloud,
  hydrateKuaishouBindingsFromCloud,
  type KuaishouCloudBindingRow,
...
const AUTO_KEY = 'meoo_kuaishou_auto_refresh'
...
function listErrorIndicatesInfrastructure(msg: string): boolean {
  ...
  if (/返回.*html|非\s*json|开放平台网页|快手生活服务开放平台/.test(c)) return true
  if (/nginx|反代|proxy_pass|自建反代|502|503|504|网关|fetch failed|econnreset|aborted|中止/i.test(m))
    return true
  if (c.includes('KUAISHOU_OPENAPI_BASE_URL') || m.includes('/douyin/')) return true
...
export default function KuaishouMerchantSection() {
  ...
  const [cloudBindings, setCloudBindings] = useState<KuaishouCloudBindingRow[]>([])
  ...
  const [rows, setRows] = useState<KuaishouStoreRow[]>([])
  ...
  /** 绑定成功后会 setAccessToken 并显式 loadStores；避免与下方 useEffect 再叠一次全量 shop.query（易触发快手限流） */
  ...
  useEffect(() => {
    ...
    const hydrate = async () => {
      ...
      const rows = await hydrateKuaishouBindingsFromCloud(sb)
  ...
  const loadStores = useCallback(
    async (opts?: {
      ...
      const res = await getKuaishouStores({
        accessToken: tok,
        page,
        pageSize,
  ...
  const selectKuaishouBinding = useCallback(
    async (bindingId: string) => {
      ...
      if (!row || row.provider !== 'kuaishou') return
      applyActiveKuaishouBinding(row)
  ...
  const removeKuaishouBinding = useCallback(
    async (bindingId: string) => {
      ...
      const rows = await hydrateKuaishouBindingsFromCloud(supabase)
      setCloudBindings(rows)
      const tok = readMerchantSession(TOKEN_KEY)
      setAccessToken(tok)
      setBoundMerchantId(readMerchantSession(META_MERCHANT_ID) ?? '')
  ...
  const handleBind = async () => {
    ...
    try {
      const r = await postKuaishouBind({
        appId: appId.trim(),
      ...
      if (supabaseConfigured && supabase) {
        ...
        if (cr.ok === false) {
          console.warn('[kuaishou] 绑定成功后云端同步失败:', cr.message)
        } else {
          const rows = await hydrateKuaishouBindingsFromCloud(supabase)
          setCloudBindings(rows)
        }
  ...
  const disconnect = () => {
    const id = readActiveBindingId('kuaishou')
    if (id) {
      void removeKuaishouBinding(id)
      return
    }
    ...
    <div className="space-y-8">
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-xl border border-gray-200 bg-white p-5">
            ...
            <p className="mt-3 text-xs leading-relaxed text-gray-500">
              绑定成功后，门店列表与账号信息来自快手团购实时接口。若为贵司私有化部署，请在实施文档中配置与服务端约定的「商户数据接口」访问方式；授权与认领流程说明见{' '}
              <a
                href="https://open.kwailocallife.com/docs/resource/zh-CN/local-life/develop/OpenAPI/general-capabilities/life.capacity.shop/auth_with_bind"
                target="_blank"
                rel="noreferrer"
      ...
      {guideOpen && (
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-labelledby="kuaishou-bind-guide-title"
        ...
        >
          ...
          >
            <div className="flex shrink-0 items-center justify-between border-b border-gray-200 px-5 py-4">
              <h3 id="kuaishou-bind-guide-title" className="text-lg font-semibold text-gray-900">
                快手团购绑定说明书
              </h3>
              <button
                type="button"
