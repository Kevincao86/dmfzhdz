    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-gray-500">
          在已绑定各平台<strong>服务商身份</strong>后，在此添加代运营客户的商家账号。商品上传、门店与经营数据将优先使用顶栏选中的客户；未选择时展示全部客户汇总视图。
        </p>
      </div>
      ...
      <MerchantPlatformAccountsPanel
        ...
        emptyHint="尚未绑定该平台的客户商家账号"
        onSelectActive={onSelectActive}
        onRemove={(id) => void onRemove(id)}
        onAddClick={() => setBindOpen(true)}
      />
      ...
      {bindOpen ? (
        <div className="rounded-xl border border-gray-200 bg-gray-50/80 p-5 space-y-4">
          <h4 className="font-medium text-gray-900">添加客户 · {plat === 'douyin' ? '抖音来客' : '快手团购'}</h4>
          <p className="text-xs text-gray-600">
            请使用服务商应用为对应商家完成授权后，将平台返回的商家 access_token（或本系统 sealed 凭证）与客户账号 ID 填入。详见设置内「API 文档」服务商接入说明。
          </p>
          ...
          <label className="block text-sm">
            <span className="text-gray-700">商家账号 ID</span>
            <input
              className="mt-1 w-full rounded-lg border border-gray-200 px-3 py-2 text-sm"
              value={merchantAccountId}
              onChange={(e) => setMerchantAccountId(e.target.value)}
              placeholder="平台商家 / 商户 ID"
            />
          </label>
          <label className="block text-sm">
          ...
          <label className="block text-sm">
            <span className="text-gray-700">授权凭证 / access_token</span>
            <SecretInput
              className="mt-1 w-full"
              value={accessToken}
              onChange={(e) => setAccessToken(e.target.value)}
              placeholder="粘贴 sealed 凭证或 token"
            />
          </label>
