...
/** 星选增值 AI 功能页面积分扣费说明 */
export function MpAddonPointsRateBadge({ kind, motionImitate, className }: Props) {
  return (
    <span
      className={
        className ??
      ...
      title={`${MP_POINTS_USAGE_KIND_LABELS[kind]}：${formatMpPointsRateLabel(kind, { motionImitate })}；优先消耗套餐 ai_video_quota 次数，超出后按积分扣费`}
    >
      {MP_POINTS_USAGE_KIND_LABELS[kind]} · {formatMpPointsRateLabel(kind, { motionImitate })}
    </span>
  )
}
