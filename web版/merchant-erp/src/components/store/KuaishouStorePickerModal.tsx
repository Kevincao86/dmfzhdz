...
import { getKuaishouStores } from '../../services/kuaishouMerchantApi'
import ModalPortal from '../ui/ModalPortal'

const EMPTY_POI_IDS: string[] = []

function readToken() {
  return readMerchantSession('meoo_kuaishou_merchant_token')
}

function readMerchantId() {
  return readMerchantSession('meoo_kuaishou_merchant_id')
}

export type KuaishouStoreRow = { id: string; name: string; address?: string }

export type KuaishouStorePickerModalProps = {
  open: boolean
  onClose: () => void
  title?: string
  ...
  onConfirm?: (poiIds: string[], rows: KuaishouStoreRow[]) => void
  onConfirmSingle?: (poiId: string | null, row: KuaishouStoreRow | null) => void
  initialPoiIds?: string[]
  initialSelectedId?: string | null
}

export default function KuaishouStorePickerModal({
  open,
  onClose,
  title,
  ...
  const [modalStores, setModalStores] = useState<KuaishouStoreRow[]>([])
  ...
  return (
    <ModalPortal open={open}>
      ...
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="kuaishou-store-picker-title"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex items-center justify-between border-b px-5 py-4">
          <h2 id="kuaishou-store-picker-title" className="text-lg font-semibold text-gray-900">
            {modalTitle}
          </h2>
...
export function KuaishouStorePickerTrigger({
  label,
  value,
  valueLabel,
  placeholder = '请选择门店',
  ...
  valueLabel: string
  placeholder?: string
  onChange: (poiId: string | null, row: KuaishouStoreRow | null) => boolean | void
  showAllOption?: boolean
  allOptionLabel?: string
  pickerTitle?: string
}) {
