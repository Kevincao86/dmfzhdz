...
export function AiAgentRecruitmentOrderDetailCard({ order }: { order: AiRecruitmentOrderDetail }) {
  ...
  return (
    <div className="mt-3 overflow-hidden rounded-xl border border-emerald-200/90 bg-white shadow-sm ring-1 ring-emerald-100/60">
      ...
      <div className="space-y-3 px-4 py-3 text-sm text-slate-700">
        ...
        >
          ...
          {order.allocation.notes ? (
            <p className="mt-1 text-xs text-slate-500">{order.allocation.notes}</p>
          ) : null}
        </div>
        {order.briefExcerpt ? (
          <p className="line-clamp-3 text-xs leading-relaxed text-slate-500">
            Brief：{order.briefExcerpt}
          </p>
        ) : null}
        <Link
          to="/recruitment"
