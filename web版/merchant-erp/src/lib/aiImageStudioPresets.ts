...
export function buildCarouselFiveMasterPromptExtra(
  ...
  return [
    ...
    aspectClamped
      ? isGpt
        ? '【防裁切·必遵】模型最长边比≤3:1，生成后会上下等比例居中裁成更扁横幅。上下边缘各约 15% 画高为危险区（只留场景延伸）；主标题、副标题、价格、图标与说明文字必须完整落在垂直中间约 70% 安全带内，禁止贴顶或贴底放大字。'
        : '【防裁切·必遵】生成后会上下等比例居中裁：上下各留约 15% 勿放文字；标题/价格/图标全部放在垂直中部安全带。'
      : '标题等中文文案整幅最多出现一处；上下边缘各留约 15% 安全留白，勿贴边裁切。',
    '标题等中文文案整幅最多出现一处；不要在画面写系统标签或编号。' +
      (allowedText.length ? `可用文案：${allowedText.join('；')}。` : '无已填文案时以场景为主、少放大字。'),
    hasOffer
      ? '可将已填优惠自然放在画面中部一处（距上下边各至少约 15% 画高）。'
      : '未填价格时不要自创价格与折扣文案。',
...
export function buildVisualStudioPrompt(
  ...
  const lines = [
    ...
    seriesMode === 'platform_carousel_five' && !carouselMaster
      ? '输出形态：超宽全景横幅（事后等分裁切），画面信息与构图须服务该营销玩法。'
      : seriesMode === 'platform_detail_page'
  ...
  if (carouselMaster && channel) {
    ...
    lines.push(
      seriesMode === 'platform_carousel_five'
        ? '整套超宽全景海报裁切后的一屏，须与相邻屏连续衔接，勿做成独立无关海报。'
