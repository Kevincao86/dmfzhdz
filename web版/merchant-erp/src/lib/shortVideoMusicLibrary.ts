/**
 * 短视频台 · 音乐/配乐库
 * 每条曲目唯一，并按场景标签做内容匹配推荐（无 Suno 等作曲 API 时的本地策略）。
 */

export type ShortVideoMusicMood =
  | '探店'
  | '美食'
  | '促销'
  | '氛围'
...
export type ShortVideoMusicTrack = {
  id: string
  title: string
  description: string
  moods: ShortVideoMusicMood[]
  /** 预览地址（商家 Web 静态） */
  previewUrl: string
  /** 关联案例 id（可选） */
  caseId?: string
...
const bed = (caseId: string) => `/short-video-bgm/${caseId}.m4a?v=bgm1`

/** 20 条互不重复、按案例内容匹配的配乐 */
export const SHORT_VIDEO_MUSIC_LIBRARY: readonly ShortVideoMusicTrack[] = [
  {
    id: 'bgm-visit-night',
    title: '夜市烟火',
    description: '轻快节拍，适合夜市/夜宵探店开场',
    moods: ['探店', '美食', '城市'],
    previewUrl: bed('case-visit-night'),
  ...
  },
  {
    ...
    moods: ['种草', '美业'],
    previewUrl: bed('case-seed-skincare'),
  ...
  },
  {
    ...
    moods: ['促销'],
    previewUrl: bed('case-promo-event'),
  ...
  },
  {
    ...
    moods: ['氛围', '探店'],
    previewUrl: bed('case-ambiance-cafe'),
  ...
  },
  {
    ...
    moods: ['短剧'],
    previewUrl: bed('case-drama-hook'),
  ...
  },
  {
    ...
    moods: ['美食'],
    previewUrl: bed('case-food-ramen'),
  ...
  },
  {
    ...
    moods: ['探店', '美食'],
    previewUrl: bed('case-visit-brunch'),
  ...
  },
  {
    ...
    moods: ['种草'],
    previewUrl: bed('case-seed-gadget'),
  ...
  },
  {
    ...
    moods: ['探店', '城市'],
    previewUrl: bed('case-queue'),
    caseId: 'case-queue',
  },
] as const

const KEYWORD_MOOD: { re: RegExp; mood: ShortVideoMusicMood }[] = [
...
export function recommendMusicForPrompt(prompt: string): ShortVideoMusicTrack[] {
  ...
  const ranked = [...SHORT_VIDEO_MUSIC_LIBRARY].sort(
...
export function findMusicTrack(id: string | null | undefined): ShortVideoMusicTrack | null {
  if (!id) return null
