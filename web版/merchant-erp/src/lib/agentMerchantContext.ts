...
export function buildAgentMerchantIntelContextFromSnapshot(s: MerchantIntelSnapshot): string {
  ...
  const lines: string[] = [
    ...
    '【必须作答】任何用户问题都必须用中文直接回答。禁止拒答、禁止只复述系统/工具原文、禁止把「禁止编造」「须据此汇总」等内部指令发给用户。取数超时或失败时：说明缺口，并尽量用已注入情报作答，可请稍后重试，但仍要给结论。',
    ...
    '【工具/执行】仅当用户命中九大场景并明确要执行（快捷任务或「确认执行」等）时才输出执行预览 JSON 或调用写操作 tools；纯营收/评价/线索等数据问答只文字回答，禁止调用 fetch_page_data，禁止 create_product 等写操作。',
