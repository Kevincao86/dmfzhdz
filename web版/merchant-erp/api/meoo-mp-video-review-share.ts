/**
 * POST /api/meoo-mp-video-review-share
...
export default async function handler(req: VercelRequest, res: VercelResponse): Promise<void> {
  try {
    ...
    if (/mp_video_review_share|does not exist|Could not find/i.test(msg)) {
      sendOpsJson(res, 503, {
        ok: false,
        error: 'video_review_share_table_missing',
        hint: '请执行迁移 20260629120000_mp_video_review_share.sql',
      })
      return
    }
    sendOpsJson(res, 500, { ok: false, error: 'video_review_share_failed', detail: msg.slice(0, 800) })
  }
}
