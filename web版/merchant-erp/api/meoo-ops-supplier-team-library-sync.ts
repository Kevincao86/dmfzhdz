/**
 * POST /api/meoo-ops-supplier-team-library-sync — 从 mpTalentMembers 同步拍摄/剪辑团队库。
 */
...
export default async function handler(req: VercelRequest, res: VercelResponse): Promise<void> {
  try {
    ...
    try {
      body = JSON.parse(rawBody(req) || '{}') as { role?: string }
    } catch {
    ...
    await io.save(data)
    sendOpsJson(res, 200, { ok: true, ...counts })
  } catch (e) {
    const msg = e instanceof Error ? e.message : String(e)
    sendOpsJson(res, 500, {
      ok: false,
      error: 'meoo_ops_supplier_team_library_sync_failed',
