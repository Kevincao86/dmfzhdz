/**
 * POST /api/meoo-ops-mp-recruitment-script-submit — 达人提交探店文稿（文件 URL 或文档链接）。
 */
...
import { applyScriptDraftToSnapshot, applyScriptSubmitToSnapshot } from '../src/lib/mpRecruitmentScriptCore.js'
...
export default async function handler(req: VercelRequest, res: VercelResponse): Promise<void> {
  try {
    ...
    const payload = {
      ...
      scriptLinkUrl: body.scriptLinkUrl,
      scriptFileName: body.scriptFileName,
    }
    const result = isDraft
      ? applyScriptDraftToSnapshot(data, mpOrderId, applicantId, payload)
      : applyScriptSubmitToSnapshot(data, mpOrderId, applicantId, payload)
    if (!result.ok) {
    ...
    sendOpsJson(res, 500, { ok: false, error: 'script_submit_failed', detail: msg.slice(0, 800) })
  }
}
