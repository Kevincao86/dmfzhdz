/**
 * 抖音来客绑定核心（单文件）：供 `douyin-bind.ts` / `meoo-douyin-bind.ts` 等同目录入口静态引用，
 * 避免 Vercel 将动态 import 指向子目录时遗漏 `api/merchant/douyin/bindRuntime` → ERR_MODULE_NOT_FOUND。
 *
 * 原 `bindShared` + `bindRuntime` 逻辑合并于此；`api/merchant/douyin/*.ts` 仅做 re-export。
 */
