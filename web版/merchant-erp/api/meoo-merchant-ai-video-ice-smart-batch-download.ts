...
/**
 * 智能一键成片下载：直连 OSS 后以 Buffer 写回 Response。
...
export default async function handler(req: VercelRequest, res: VercelResponse): Promise<void> {
  ...
  const batchJobId = url.searchParams.get('id')?.trim()
  if (!batchJobId) {
    sendMerchantJson(res, 400, { ok: false, message: '缺少 id' })
    return
  }

  const { fetchIceSmartBatchDownloadBuffer, loadIceGatewayConfig } = await import(
  ...
  const payload = await fetchIceSmartBatchDownloadBuffer(cfg, batchJobId)
  if (!payload.ok) {
