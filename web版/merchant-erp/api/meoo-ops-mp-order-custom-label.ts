/**
 * POST /api/meoo-ops-mp-order-custom-label
 * - ECS：handleMpOrderCustomLabelBody（Postgres 商单标签表）
 * - Vercel：代拉 ECS
 */
...
import {
  handleMpOrderCustomLabelBody,
  ownerFromMpAccount,
  type MpOrderCustomLabelBody,
} from '../src/lib/mpOrderCustomLabelCore.js'
import {
...
export default async function handler(req: VercelRequest, res: VercelResponse): Promise<void> {
  try {
    ...
    if (isVercelServerless()) {
      ...
      const out = await proxyPostErpApi('/api/meoo-ops-mp-order-custom-label', proxyBody)
    ...
    const out = await handleMpOrderCustomLabelBody(body as MpOrderCustomLabelBody, auth)
    sendOpsJson(res, out.status, out.data)
  } catch (e) {
    const msg = e instanceof Error ? e.message : String(e)
    sendOpsJson(res, isVercelServerless() ? 503 : 500, {
      ok: false,
      error: 'meoo_ops_mp_order_custom_label_failed',
