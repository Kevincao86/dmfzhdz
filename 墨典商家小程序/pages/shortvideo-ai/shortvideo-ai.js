...
Page({
  ...
  async runLongformGenerate() {
    ...
    for (let i = 0; i < plan.prompts.length; i++) {
      ...
      if (this.data.engine === 'kling') {
        ...
        this.setData({ err: '长视频合成当前在小程序端仅完整支持墨典视频模型1；请切换模型或使用电脑端。' })
        return
      }
      segmentUrls.push(urlOut)
      lastUrl = urlOut
  ...
  pickIceVideo() {
    if (!this.data.iceLocalUpload) {
      this.setData({
        iceErr:
          '本地上传尚未开启：请运营在「商家管理后台 → AI模型 → 短视频 API → 墨典AI云剪」填写 OSS 成片 URL 前缀并保存，然后刷新本页。仍可粘贴下方 HTTPS 链接作为素材。',
      })
      return
    }
    wx.chooseMedia({
  ...
  async runOneClickImages() {
    ...
    if (!cfg || !cfg.configured) {
      this.setData({ iceErr: '墨典AI云剪服务未就绪' })
      return
    }
