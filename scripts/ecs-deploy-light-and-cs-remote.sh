#!/usr/bin/env bash
# 本机一键：推送后 SSH 部署轻量（API+根域 Web）与新 ECS（cs 子域）
#
# 用法:
...
#
# 仅轻量或仅 cs:
#   DEPLOY_LIGHT=1 DEPLOY_CS=0 bash scripts/ecs-deploy-light-and-cs-remote.sh
...
echo "部署目标: 轻量=$LIGHT_HOST (DEPLOY_LIGHT=$DEPLOY_LIGHT), cs ECS=$CS_HOST (DEPLOY_CS=$DEPLOY_CS)"
