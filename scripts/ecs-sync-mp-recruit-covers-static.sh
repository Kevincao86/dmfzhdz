...
if [[ -d "$MP/images/identity" ]]; then
  sudo cp -f "$MP/images/identity"/*.png "$STATIC_ROOT/identity/" 2>/dev/null || true
fi
if [[ -d "$ROOT/灵祺达人履约管理后台/public/recruit-covers/identity" ]]; then
