...
# 轻量 SSH：cd ~/app && bash scripts/ecs-git-pull-gitee.sh && bash scripts/ecs-sync-merchant-notify-posters.sh
# 本机远程：bash scripts/ecs-sync-merchant-notify-posters.sh --remote
set -euo pipefail
...
set -euo pipefail
cd ~/app && bash scripts/ecs-git-pull-gitee.sh && bash scripts/ecs-sync-merchant-notify-posters.sh
EOF
else
  run_sync
fi
