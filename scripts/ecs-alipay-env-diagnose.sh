...
echo ""
if [[ -f "$ENV" ]]; then
  for k in ALIPAY_APP_ID ALIPAY_APPID ALIPAY_NOTIFY_URL ALIPAY_RETURN_URL ALIPAY_PAY_PRODUCT ALIPAY_PRIVATE_KEY_FILE ALIPAY_PUBLIC_KEY_FILE; do
...
echo ""
echo "探活（公网，含 page.pay 探针）："
curl -sS "https://mofangdianai.com/erp-api/meoo-alipay-pay-notify?detail=1&probePay=1" 2>/dev/null || echo "curl 失败"
echo ""
