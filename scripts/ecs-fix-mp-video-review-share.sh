#!/usr/bin/env bash
# 轻量：分享审片表建表 + 关闭 RLS（create 500 / db_error / () 时执行）
# ECS: cd ~/app && git pull && bash scripts/ecs-fix-mp-video-review-share.sh
set -euo pipefail
...
verify_postgrest() {
  ...
  curl -sS -m 8 -o /dev/null -w "GET links HTTP %{http_code}\n" \
    ...
    "http://127.0.0.1:8888/rest/v1/mp_video_review_share_links?select=id&limit=1" || true
}
...
for mig in \
  20260629120000_mp_video_review_share.sql \
  20260630120000_mp_video_review_share_ecs_rls.sql
do
...
echo "=== 接口探活 create（替换为真实 mpOrderId）==="
curl -sS -m 15 -X POST "http://127.0.0.1:${AUTH_PORT}/api/meoo-mp-video-review-share" \
  -H 'Content-Type: application/json' \
  -d '{"action":"list_feedback","mpOrderId":"MP-RO-probe"}' | head -c 240
echo
echo "OK: 分享审片表修复完成。请用真实订单再测 create。"
